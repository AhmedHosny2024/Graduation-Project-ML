...
def detect_objects(image, classifier):
    _, regions = selectivesearch.selective_search(image, min_size=1500)
    detected_boxes = []
...
for idx in range(len(data_info)):
    ...
    for region in regions:
        ...
        for gt_box in scaled_gt_boxes:
            if calculate_iou((x, y, w, h), gt_box) > 0.3:  # Consider a proposal as positive if IoU > 0.5
                label = 1
                break
        labels.append(label)
...
print("length of detected_boxes_label: ",len(detected_boxes_label))
...
plt.axis('off')
plt.show()
# python -m ObjectDetector.selective_haar_svm 